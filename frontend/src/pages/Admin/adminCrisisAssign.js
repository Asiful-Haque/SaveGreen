...
const CrisisAssign = () => {
    ...
    const { crisis_id } = location.state || {}; //extract the state value
    // Handle the assign all action
        ...
        if (selectedVolunteers.length > 0) {
            try {
                const res = await fetch("http://localhost:5000/api/crisis/assign_volunteers", {
                    ...
                    body: JSON.stringify({
                        volunteer_ids: selectedVolunteers, // Send the selected volunteers to the server
                        crisis_id: crisis_id //send crisis id that found as state
                    }),
                });
                if (res.ok) {
                    console.log("Volunteers successfully assigned!");
                    // resetting the selectedVolunteers after successful assignment
                    setSelectedVolunteers([]);
                } else {
                    console.error("Failed to assign volunteers");
                    alert("There is a problem.Maybe Its already added");
                }
            } catch (error) {
